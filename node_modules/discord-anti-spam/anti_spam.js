...
const muteRole = bot.guilds.get(msg.guild.id).roles.find('name', `SendMessage`);
const modlog = bot.channels.find('name', 'mod-log');
  ...
  .setFooter(`User: ${msg.author.username}`,`${msg.author.avatarURL}`);
    if(msg.author.id != bot.user.id){
      ...
      if (msgMatch == maxDuplicatesBan && !banned.includes(msg.author.id) && msg.guild.member(msg.author).roles.has(muteRole.id))  {

        msg.guild.member(msg.author).removeRole(muteRole)
    bot.channels.get(modlog.id).send({embed}).catch(console.error);
     msg.channel.send({embed}).catch(console.error);
          msg.author.sendMessage({embed: {
color: 0xff0000,
title: "User Automuted Warning",
description: (`\nYou have been auto muted!\n`),
fields: [{
  name : "Username:",
  value : `${msg.author.username} (${msg.author})`,
  inline : true
},{
  name : "Moderator:",
  value :`Pie Bot`,
  ...
  name : "Mute reason:",
  value : `[Auto-Mute] Auto muted by system for spamming. (1 day)`,
  inline : false
},{
